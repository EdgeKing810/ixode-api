...
use crate::{components::{routing::submodules::sub_ref_data::RefData, constraint_property::ConstraintProperty}, utils::{mapping::auto_fetch_all_mappings, constraint::auto_fetch_all_constraints}};
...
impl LoopBlock {
    ...
    pub fn get(all_blocks: &Vec<LoopBlock>, global_index: u32) -> Option<LoopBlock> {
        for block in all_blocks.iter() {
            if block.global_index == global_index {
    ...
    pub fn update_local_name(
        ...
        let final_value = match ConstraintProperty::validate(&all_constraints, "loop_block", "local_name", local_name) {
            Ok(v) => v,
            Err(e) => return Err(e),
        };
