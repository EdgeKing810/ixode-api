...
pub async fn main(
    ...
    let allowed_collections = Collection::get_all(&all_collections, &passed_project_id);

    let amount = allowed_collections.len();
    let processed_collections = paginate(allowed_collections, passed_limit, passed_offset);

    return json!({"status": 200, "message": "Collections successfully fetched!", "collections": processed_collections, "amount": amount});
}
