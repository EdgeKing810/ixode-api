    ...
    pub fn add_definition(
        ...
        if block_name == "BODY" {
            ...
            if res_condition {
                let mappings = auto_fetch_all_mappings();
                let all_data =
                    match auto_fetch_all_data(&mappings, &project_id, &create_block.ref_col) {
                        Ok(d) => d,
                        _ => {
                            return Err((500, String::from("Error: Failed fetching data")));
                        }
                ...
                let collection =
                    match Collection::get(&all_collections, project_id, &create_block.ref_col) {
                        Ok(p) => p,
                        Err(_) => {
                            return Err((
                                404,
                ...
                match resolve_raw_data(
                    ...
                    Ok(def) => match def {
                        DefinitionData::DATA(d) => {
                            actual_definition = DefinitionStore {
                                block_name: block_name.to_string(),
                                ref_name: format!("create_{}_{}", create_block.ref_col, index),
                                index: index,
                                data: DefinitionData::DATA(d.clone()),
                            };
